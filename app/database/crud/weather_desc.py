...
def create_weather_desc(db: Session, code: int, desc: str) -> Weather_desc:

    new_weather_desc = Weather_desc(code=code, desc=desc)
...
def update_weather_desc(db: Session, code: int, desc: str) -> Optional[Weather_desc]:
    weather_desc = db.query(Weather_desc).filter(Weather_desc.id == id).first()

    if not weather_desc:
        return None
    
    weather_desc.code = code
